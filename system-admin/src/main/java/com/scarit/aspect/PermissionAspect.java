...
import com.scarit.exception.PermissionNeedHasException;
import com.scarit.exception.RoleNeedHasException;
...
public class PermissionAspect {
    ...
    public void roleBefore(JoinPoint joinPoint, HasPermission hasPermission){
        ...
        HttpServletRequest request = ((ServletRequestAttributes) Objects.requireNonNull(RequestContextHolder.getRequestAttributes())).getRequest();
        String token = request.getHeader(Constants.HEAD_AUTHORIZATION);
        List<String> hasPermissions = redisTemplate.getObject(Constants.PERMS_PREFIX + token, new TypeReference<>() {
        });
        //只要所需的角色有一个在你拥有的角色中就放行
        boolean flag = false;
        ...
        if(!flag) throw new PermissionNeedHasException("您没有该接口所需要的权限");
    }
